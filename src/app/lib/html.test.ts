...
describe('stripHtml — common README HTML', () => {
  ...
  test('details/summary collapses to concatenated text', () => {
    expect(stripHtml('<details><summary>Toggle</summary>Hidden body</details>')).toBe(
      'TogglehiddenBody'.replace('hiddenBody', 'Hidden body'),
    )
  })

  test('details with open attribute behaves the same', () => {
    expect(stripHtml('<details open><summary>S</summary>B</details>')).toBe('SB')
  })
